...
namespace mz{
    ...
    pivot find_pivot(Matrix *M, int row_begin, int col_begin) {
        ...
        for (int i = row_begin;
             i < M->rows; i++) {            // For partial pivot i = row_begin and there isn't this loop
    ...
    LU_dec::LU_dec(const Matrix &L, const Matrix &U, vector<double> b) {
        this->L = L;
        this->U = U;
        this->b = b;
    }
    ...
    double find_maximum(vector<double> &v) {
        double m = v[0];
    ...
    void multiple_row(vector<double> &row_v, std::vector<double> &b, int row, pivot p) {
        double m = find_maximum(row_v);
        double multiply = p.val / m;
        double mul = 1.0;
        while (multiply > 10.0) {
            mul *= 10;
            multiply /= 10;
        }
        for (auto it = row_v.begin(); it != row_v.end(); it++) {
            (*it) = (*it) * mul;
        }
        b[row] *= mul;
    }

    void scale(Matrix &M, std::vector<double> &b) {
        pivot p = find_partial_pivot(&M, 0, 0);
        for (int r = 0; r < M.rows; r++) {
            if (r != p.row) {
                multiple_row(M.v[r], b, r, p);
            }
        }
    }
}